...
export { extractBrandModelFromTitle } from "./brandFromTitle";
...
function organicToListing(
  ...
  if (
    visionBrand &&
    parsedBrand &&
    brandsConflict(visionBrand, parsedBrand, title)
  ) {
    console.log(
      `[DEBUG][GoogleSearchApify] organic REJECT brand_vision_conflict vision="${visionBrand}" parsed="${parsedBrand}" title="${title.slice(0, 80)}"`
    );
    return null;
  }
  ...
  if (
    !brand &&
    visionBrand &&
    titleMentionsBrand(title, visionBrand)
  ) {
    brand = visionBrand;
  }
...
async function runGoogleSearch(
  ...
  if (canonical.model && !aggregated.model) {
    aggregated.model = canonical.model;
  }
  validateBrandTitleConsistency(
    repTitle,
    String(aggregated.brand ?? ""),
    "GoogleSearchApify"
  );

  const aggSources = aggregated.fieldSources as
    | Record<string, string>
    | undefined;
  console.log("[GoogleSearchApify] fieldSources:", {
    title: aggSources?.title ?? "aggregated.title",
    brand: brandResolution.source,
    brandCorrected: brandResolution.corrected,
    model: canonical.model ? "canonical.model" : "aggregated.model",
  });
  const repListing =
