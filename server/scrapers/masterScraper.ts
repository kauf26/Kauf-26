...
export const scrapeProduct = async (query: string): Promise<any> => {
  ...
  // 1. Use allSettled to ensure individual failures don't crash the process
  const results = await Promise.allSettled(scrapers.map(s => s(query)));

  // DEBUGGING: Log each result to see exactly what is being returned
  results.forEach((res, index) => {
    if (res.status === 'fulfilled') {
      console.log(`[MasterScraper] Scraper ${index} succeeded with:`, JSON.stringify(res.value));
    } else {
      console.log(`[MasterScraper] Scraper ${index} failed:`, res.reason);
    }
  });
