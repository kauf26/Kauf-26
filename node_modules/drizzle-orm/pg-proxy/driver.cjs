...
=======
  const session = new import_session.PgRemoteSession(callback, dialect, schema, { logger, cache: config.cache });
  const db = new PgRemoteDatabase(dialect, session, schema);
